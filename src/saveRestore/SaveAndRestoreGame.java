...
public class SaveAndRestoreGame {
	...
	public void saveGame(FileGame fileGame) {
		...
		System.out.println("Salvando fileGame: " + fileGame);
	}
	
	public FileGame loadGame(File file) {
		
		//TODO - ler o file e montar pieces e player turn
		// (e talvez posicoes finais)
		
		FileGame fileGame = new FileGame();
		List<Piece> pieces = simulatePieces();
		
		fileGame.setPieces(pieces);
		fileGame.setFile(file);
		fileGame.setPlayerTurn(Color.RED);
		fileGame.setLastMovedPiece(new Piece(1, 40, Color.YELLOW,false));
//		fileGame.setBoardSpaces(new ArrayList<BoardSpace>())
		return fileGame;
